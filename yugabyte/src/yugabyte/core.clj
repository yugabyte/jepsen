...
(def workloads-ysql
  ...
  #:ysql{:none               noop-test
         ...
         :sz.append-table    (fn [opts] (assoc (append/workload-serializable opts)
                                            :client (ysql.append-table/->Client :serializable)
                                            :concurrency 2))
         ...
         :rc.append-table    (fn [opts] (assoc (append/workload-rc opts)
                                            :client (ysql.append-table/->Client :read-committed)
                                            :concurrency 2))
         ...
         :si.append-table    (fn [opts] (assoc (append/workload-si opts)
                                            :client (ysql.append-table/->Client :repeatable-read)
                                            :concurrency 2))
